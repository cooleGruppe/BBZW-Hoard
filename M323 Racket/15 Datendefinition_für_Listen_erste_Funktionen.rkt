...
#reader(lib "beginner-reader.rkt" "deinprogramm" "sdp")((modname 15_Datendefinition_für_Listen_erste_Funktionen) (read-case-sensitive #f) (teachpacks ((lib "image.rkt" "teachpack" "deinprogramm" "sdp"))) (deinprogramm-settings #(#f write repeating-decimal #f #t none explicit #f ((lib "image.rkt" "teachpack" "deinprogramm" "sdp")))))
; ----- Listendefinition -----
...
(define listOdd (cons 3 (cons 5 empty))) ; In dieser Liste befinden sich positive und negative Zahlen
...
(define odd
  (lambda (list)
    (cond
      ...
      ((cons? list)
       (define f (first list))
       (define r (odd (rest list)))
       (if (odd? f)
           (cons f r)
           r)))))

(check-expect (odd listOdd) (cons 3 (cons 5 empty)))

...
#reader(lib "beginner-reader.rkt" "deinprogramm" "sdp")((modname 18_Listen_abstrahieren) (read-case-sensitive #f) (teachpacks ((lib "image.rkt" "teachpack" "deinprogramm" "sdp"))) (deinprogramm-settings #(#f write repeating-decimal #f #t none explicit #f ((lib "image.rkt" "teachpack" "deinprogramm" "sdp")))))
...
(define list-length
  (lambda (list)
    (cond
      ((empty? list) make-empty)
      (else (+ 1 (list-length (rest list)))))))


; Examples
(define _list0 make-empty)                 ; An empty list
(define _list1 (cons 17 empty))       ; A singleton list: 17 (+ empty)
(define _list2 (cons 5 (cons 17 empty))) ; A 2-element list: 5 17

; Test cases
(check-expect (list-length _list0) 0)
(check-expect (list-length _list1) 1)
(check-expect (list-length _list2) 2)
